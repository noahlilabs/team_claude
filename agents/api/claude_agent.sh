...
export CLAUDE_agent3="agent3"
export CLAUDE_backend="backend"
export CLAUDE_python,api,database,server="python,api,database,server"
export CLAUDE_feature-api="feature-api"
export CLAUDE_STATE_FILE="$REPO_ROOT/claude_state.json"
export CLAUDE_/Users/noahlofquist/Desktop/multi-agent claude/agents/api="/Users/noahlofquist/Desktop/multi-agent claude/agents/api"
...
echo "Starting Claude Agent: $CLAUDE_agent3 (Type: $CLAUDE_backend)"
echo "Capabilities: $CLAUDE_python,api,database,server"
echo "Working directory: $CLAUDE_/Users/noahlofquist/Desktop/multi-agent claude/agents/api"
cd "$CLAUDE_/Users/noahlofquist/Desktop/multi-agent claude/agents/api" || exit 1

# Log that the agent is starting up
TIMESTAMP=$(date +"%Y-%m-%d %H:%M:%S")
echo "[$TIMESTAMP] Agent $CLAUDE_agent3 starting up..."

# Register agent with the state manager
echo "[$TIMESTAMP] Registering agent $CLAUDE_agent3 with capabilities: $CLAUDE_python,api,database,server"
python3 "$REPO_ROOT/src/cli/state_cli.py" register_agent "$CLAUDE_agent3" "$CLAUDE_backend" "$CLAUDE_python,api,database,server"
if [ $? -ne 0 ]; then
...
python3 "$REPO_ROOT/src/cli/state_cli.py" log_reasoning "$CLAUDE_agent3" "startup" "Agent started successfully with capabilities: $CLAUDE_python,api,database,server" --tags "startup"

echo "[$TIMESTAMP] Registered agent $CLAUDE_agent3 with the system"
...
while true; do
    ...
    echo "[$TIMESTAMP] Looking for tasks for branch $CLAUDE_feature-api:"
    BRANCH_TASKS=$(python3 "$REPO_ROOT/src/cli/state_cli.py" get_tasks "$CLAUDE_feature-api")
    echo "$BRANCH_TASKS"
    ...
    echo "[$TIMESTAMP] Looking for pending tasks assigned to $CLAUDE_agent3:"
    PENDING_TASKS=$(python3 "$REPO_ROOT/src/cli/state_cli.py" get_tasks --agent "$CLAUDE_agent3" --status "pending")
    ...
    # Then look for pending tasks
        ...
        echo "[$TIMESTAMP] No pending tasks found for $CLAUDE_agent3."
    fi
