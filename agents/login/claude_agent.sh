...
export CLAUDE_agent1="agent1"
export CLAUDE_frontend="frontend"
export CLAUDE_html,css,javascript,auth="html,css,javascript,auth"
export CLAUDE_feature-login="feature-login"
export CLAUDE_STATE_FILE="$REPO_ROOT/claude_state.json"
export CLAUDE_/Users/noahlofquist/Desktop/multi-agent claude/agents/login="/Users/noahlofquist/Desktop/multi-agent claude/agents/login"
...
echo "Starting Claude Agent: $CLAUDE_agent1 (Type: $CLAUDE_frontend)"
echo "Capabilities: $CLAUDE_html,css,javascript,auth"
echo "Working directory: $CLAUDE_/Users/noahlofquist/Desktop/multi-agent claude/agents/login"
cd "$CLAUDE_/Users/noahlofquist/Desktop/multi-agent claude/agents/login" || exit 1

# Log that the agent is starting up
TIMESTAMP=$(date +"%Y-%m-%d %H:%M:%S")
echo "[$TIMESTAMP] Agent $CLAUDE_agent1 starting up..."

# Register agent with the state manager
echo "[$TIMESTAMP] Registering agent $CLAUDE_agent1 with capabilities: $CLAUDE_html,css,javascript,auth"
python3 "$REPO_ROOT/src/cli/state_cli.py" register_agent "$CLAUDE_agent1" "$CLAUDE_frontend" "$CLAUDE_html,css,javascript,auth"
if [ $? -ne 0 ]; then
...
python3 "$REPO_ROOT/src/cli/state_cli.py" log_reasoning "$CLAUDE_agent1" "startup" "Agent started successfully with capabilities: $CLAUDE_html,css,javascript,auth" --tags "startup"

echo "[$TIMESTAMP] Registered agent $CLAUDE_agent1 with the system"
...
while true; do
    ...
    echo "[$TIMESTAMP] Looking for tasks for branch $CLAUDE_feature-login:"
    BRANCH_TASKS=$(python3 "$REPO_ROOT/src/cli/state_cli.py" get_tasks "$CLAUDE_feature-login")
    echo "$BRANCH_TASKS"
    ...
    echo "[$TIMESTAMP] Looking for pending tasks assigned to $CLAUDE_agent1:"
    PENDING_TASKS=$(python3 "$REPO_ROOT/src/cli/state_cli.py" get_tasks --agent "$CLAUDE_agent1" --status "pending")
    ...
    # Then look for pending tasks
        ...
        echo "[$TIMESTAMP] No pending tasks found for $CLAUDE_agent1."
    fi
